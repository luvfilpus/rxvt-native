 ...
 * File:	init.c
 *----------------------------------------------------------------------*
 * $Id: init.c,v 1.123 2004/01/29 23:54:19 gcw Exp $
 *
...
/* EXTPROTO */
void
rxvt_set_unix_alt_space(rxvt_t *r)
{
    if (r->Options & Opt_unixAltSpace)
      set_unix_alt_space (True);
    else
      set_unix_alt_space (False);
}

/*----------------------------------------------------------------------*/
/* EXTPROTO */
void
rxvt_change_directory(rxvt_t *r)
{
/*
 ...
 */
    ...
    if (r->h->rs[Rs_changeDirectory] != NULL) {
      len = STRLEN(r->h->rs[Rs_changeDirectory]);
      if (len >= PATH_MAX) {
        rxvt_print_error("Path too long");
        return;
      }
      posix_path = malloc((PATH_MAX+1) * sizeof (char));
      if (!posix_path) {
        rxvt_print_error(strerror(errno));
        return;
      }
      posix_path[0]='\0';
      posix_path[PATH_MAX]='\0';

#if defined(__CYGWIN__)
      /* on a UNC drive the first backslash is lost!!
       * We assume that posix style absolute directories
       * will start with '/', not '\\'; therefore, if a
       * win32-style directory begins with a single backslash
       * we will assume it is intended to be a UNC path and
       * should have two backslashes. This means it is not
       * possible to specify a path relative to the root of
       * the current drive, without explicitly specifying the
       * drive: e.g. \top\of\current\drive must be specified as
       * C:\top\of\specific\drive.
       */
      tmp_path = malloc((PATH_MAX+1) * sizeof (char));
      if (!tmp_path) {
        rxvt_print_error(strerror(errno));
        free(posix_path);
        return;
      ...
      STRCPY(tmp_path, r->h->rs[Rs_changeDirectory]);
      if (r->h->rs[Rs_changeDirectory][0] == '\\' &&
          r->h->rs[Rs_changeDirectory][1] != '\\') {
        if (len+1 >= PATH_MAX) {
          rxvt_print_error("Path too long");
          free (tmp_path);
          free(posix_path);
        ...
        memmove(tmp_path+1, tmp_path, len+1); // include trailing '\0'
        len++;
      }
#else
      tmp_path = r->h->rs[Rs_changeDirectory];
#endif

#if HAVE_DECL_CYGWIN_CONV_PATH
      if (cygwin_conv_path (CCP_WIN_A_TO_POSIX, tmp_path, posix_path, PATH_MAX)) {
        rxvt_print_error(strerror(errno));
        free(posix_path);
        free(tmp_path);
        return;
      ...
      cygwin_conv_to_full_posix_path(tmp_path, posix_path);
#else
      STRCPY(posix_path, tmp_path);
#endif
      if (chdir(posix_path))
        rxvt_print_error(strerror(errno));
...
rxvt_Get_Colours(rxvt_t *r)
{
    ...
    r->h->color_origbg = r->PixColors[Color_bg];
...
/* signal (SIGURG, SIG_DFL); */
    ...
    /* set window size */
    rxvt_tt_winsize(r->tty_fd, r->TermWin.ncol, r->TermWin.nrow, 0);
