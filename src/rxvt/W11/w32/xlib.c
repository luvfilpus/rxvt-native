...
const char *name;
{
	...
	initQ();
	SYSLOG ((LOG_NOTICE, "XOpenDisplay"));
	...
	/* the following only works because a pointer fits into a CARD32 */
	scrd->root = (Window)NT_new_window();
...
XCloseDisplay(Display *display)
{
  ...
  SYSLOG ((LOG_NOTICE, "XCloseDisplay"));
  closelog ();
...
Window *ch;
{
	...
	pt.x = sx;
	pt.y = sy;
	ClientToScreen((HWND)(ntsw->w), &pt);
	ScreenToClient((HWND)(ntdw->w), &pt);
	*dx = pt.x;
	*dy = pt.y;

        win = ChildWindowFromPointEx((HWND)(ntdw->w), pt,
                 CWP_SKIPINVISIBLE|CWP_SKIPDISABLED|CWP_SKIPTRANSPARENT);
        if (win && (win != (HWND)(ntdw->w)))
        {
                Window w = (Window)NT_find_window_from_id(win);
		if (!w)
			w = None;
		*ch = w;
        }
    ...
    XColor *colorPtr;
{
	...
	if ((i = strlen(spec+4))%3 != 2) {
	    return 0;
	}
	i /= 3;

	sprintf(fmt, "%%%dx/%%%dx/%%%dx", i, i, i);
	if (sscanf(spec+4, fmt, &red, &green, &blue) != 3) {
	    return 0;
	}
	colorPtr->red = ((unsigned short) red) << (4 * (4 - i));
	colorPtr->green = ((unsigned short) green) << (4 * (4 - i));
	...
	if (sscanf(spec+5, "%lf/%lf/%lf", &ri, &gi, &bi) != 3) {
	    return 0;
	}
	if (ri > 1.0)
	    ri = 1.0;
	...
	if (bi > 1.0)
	    ...
	    bi = 0.0;
	colorPtr->red = (unsigned short) ((double)((unsigned short)-1) * ri);
	colorPtr->blue = (unsigned short) ((double)((unsigned short)-1) * bi);
	colorPtr->green = (unsigned short) ((double)((unsigned short)-1) * gi);
    } else if (!FindColor(spec, colorPtr)) {
	return 0;
    }
