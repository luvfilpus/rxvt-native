 ...
 * File:	main.c
 *----------------------------------------------------------------------*
 * $Id: main.c,v 1.180 2004/03/10 08:01:01 gcw Exp $
 *
...
/* load font or substitute */
	...
	for (; i < (fh - r->TermWin.lineSpace) / 2; i++) {
	    sprintf(tmpbuf, c, fh - i - r->TermWin.lineSpace);
...
rxvt_set_iconName(rxvt_t *r, const char *str)
{
    ...
    if (XGetIconName(r->Xdisplay, r->TermWin.parent[0], &name) == 0)
	name = NULL;
    if (name == NULL || STRCMP(name, str))
	XSetIconName(r->Xdisplay, r->TermWin.parent[0], str);
    if (name)
