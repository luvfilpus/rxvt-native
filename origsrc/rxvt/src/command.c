 ...
 * File:	command.c
 *----------------------------------------------------------------------*
 * $Id: command.c,v 1.278 2004/01/29 23:54:18 gcw Exp $
 *
...
/* INTPROTO */
XButtonEvent *
rxvt_swap_buttons(rxvt_t *r, XButtonEvent *ev)
{
    if (r->Options & Opt_swapButtons)
...
/* INTPROTO */
void
rxvt_set_bg_focused(rxvt_t *r, Bool focus)
{
    XColor          xcol;
    XGCValues       gcvalue;
    ...
    gcvalue.background = focus ? r->h->color_origbg : r->PixColors[Color_ufbg];
    r->PixColors[Color_bg] = gcvalue.background;

#if defined(TRANSPARENT) || defined(XPM_BACKGROUND)
    if (!focus
	|| (!(r->Options & Opt_transparent) && r->TermWin.pixmap == None))
#endif
    {
	XChangeGC(r->Xdisplay, r->TermWin.gc, GCBackground, &gcvalue);
	XSetWindowBackground(r->Xdisplay, r->TermWin.vt, r->PixColors[Color_bg]);
    }
#if defined(TRANSPARENT) || defined(XPM_BACKGROUND)
    else if (r->Options & Opt_transparent)
	rxvt_check_our_parents(r);
    else if (r->TermWin.pixmap != None)
...
rxvt_process_x_event(rxvt_t *r, XEvent *ev)
{
    ...
    case ButtonPress:
	rxvt_button_press(r, rxvt_swap_buttons(r, (XButtonEvent *)ev));
	break;

    case ButtonRelease:
	rxvt_button_release(r, rxvt_swap_buttons(r, (XButtonEvent *)ev));
	break;
